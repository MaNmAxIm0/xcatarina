...
function downloadLabel(url: string, ratio: "Horizontal" | "Vertical") {
  ...
  return <div className="styled-player" ref={shellRef}>
    ...
    <div className="player-controls"><button type="button" onClick={togglePlay} aria-label={playing ? "Pausar vídeo" : "Reproduzir vídeo"}>{playing ? "Ⅱ" : "▶"}</button><input aria-label="Progresso do vídeo" type="range" min="0" max="100" step="0.1" value={progress} onChange={(event) => seek(Number(event.target.value))} /><button className="fullscreen-button" type="button" onClick={fullscreen} aria-label="Ecrã inteiro" title="Ecrã inteiro">⛶</button></div>
  </div>;
}
