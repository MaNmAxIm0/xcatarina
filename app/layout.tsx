...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const host = h.get("x-forwarded-host") || h.get("host") || "xcatarina.pt";
  const protocol = h.get("x-forwarded-proto") || (host.includes("localhost") ? "http" : "https");
  const base = new URL(`${protocol}://${host}`);
  const title = "xCatarina — Timelapses de Arte & LEGO";
  const description = "Vê as criações das lives da xCatarina ganharem vida, da primeira peça ao último detalhe.";
  return {
    metadataBase: base,
    title,
    description,
